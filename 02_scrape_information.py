...
def scrape(show_name):
    ...
    try:
        show_tags.append(driver.find_element_by_xpath('//*[@id="titleStoryLine"]/div[2]').text)
    except:
        show_tags.append('')
        pass
...
#file1 = '/NetflixViewingHistory -Parents account.csv'
#file2 = '/NetflixViewingHistory -Zoe account.csv'
#file3 = '/NetflixViewingHistory.csv'

file = '/bryan_viewing.csv'

n = 0

#df1 = pd.read_csv(directory + file1)
#df2 = pd.read_csv(directory + file2)
#df3 = pd.read_csv(directory + file3)

bv = pd.read_csv(directory + file)

#df1['Date_Formatted'] = [*map(lambda x : dt.strptime(x, '%d/%m/%y').strftime('%Y-%m-%d'), df1['Date'])]
#df2['Date_Formatted'] = [*map(lambda x : dt.strptime(x, '%m/%d/%y').strftime('%Y-%m-%d'), df2['Date'])]
#df3['Date_Formatted'] = [*map(lambda x : dt.strptime(x, '%d/%m/%Y').strftime('%Y-%m-%d'), df3['Date'])]

#df = pd.concat([df1, df2, df3], keys = ['parent_account', 'zoe_account', 'raena_account'], ignore_index = True)
...
bv['Show'] = bv['Title'].str.split(':', -1, True)[0]

bv = bv[bv['Title'] != ' ']

unique_shows = list(bv['Show'].unique())
